...
import { cropType } from "../formtype/EquipmentType";
...
const CropTypeForm: React.FC<CropTypeProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [submitting, setSubmitting] = useState(false);
  const formField = data?._id ? populateFormFields(cropType, data) : cropType;

  const [formData, setFormData] = useState<any>(
    data?._id ? populateFormData(cropType, data) : {}
  );
  ...
  return (
    <div>
      <h2 className="text-2xl pb-4 font-semibold text-gray-800">
        {data?._id ? "Edit Crop Details" : "Add New Crop"}
      </h2>
      <DynamicForm
        fields={formField}
        formData={formData}
