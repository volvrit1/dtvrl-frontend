...
import { ServiceType } from "../formtype/EquipmentType";
...
const ServiceTypeForm: React.FC<ServiceTypeProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [submitting, setSubmitting] = useState(false);
  const formField = data?._id
    ? populateFormFields(ServiceType, data)
    : ServiceType;

  const [formData, setFormData] = useState<any>(
    data?._id ? populateFormData(ServiceType, data) : {}
  );
  ...
  return (
    <div>
      <h2 className="text-2xl pb-4 font-semibold text-gray-800">
        {data?._id ? "Edit Service Details" : "Add New Service"}
      </h2>
      <DynamicForm
        fields={formField}
        formData={formData}
