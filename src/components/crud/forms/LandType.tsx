...
import { LandType } from "../formtype/LandType";
...
const LandTypeForm: React.FC<LandTypeProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [formField, setFormField] = useState<any>(
    data?._id ? populateFormFields(LandType, data) : LandType
  );

  const [formData, setFormData] = useState<any>(
    data?._id ? populateFormData(LandType, data) : {}
  );

  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const url = "/api/user/public";
      ...
      {!loading && (
        <>
          <h2 className="text-2xl pb-4 font-semibold text-gray-800">
            {data?._id ? "Edit Land Details" : "Add New Land"}
          </h2>
          <DynamicForm
            fields={formField}
            formData={formData}
