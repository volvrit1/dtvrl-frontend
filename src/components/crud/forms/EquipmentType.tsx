...
import { EquipmentType } from "../formtype/EquipmentType";
...
const EquipmentTypeForm: React.FC<EquipmentTypeProps> = (props: any) => {
  const data = props.data;
  const formType = props.formType;
  const [submitting, setSubmitting] = useState(false);
  const formField = data?._id
    ? populateFormFields(EquipmentType, data)
    : EquipmentType;

  const [formData, setFormData] = useState<any>(
    data?._id ? populateFormData(EquipmentType, data) : {}
  );
